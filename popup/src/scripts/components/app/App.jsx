...
class App extends Component {
  ...
  render() {
    const timeLimitInMinutesOrNaN = timeLimitStringToMinutes(
      this.getTimeLimitString()
    )
          ...
          >
            <div>
              <button
                style={{ fontSize: 20 }}
                onClick={() => this.start()}
                disabled={isNaN(timeLimitInMinutesOrNaN)}
              >
                Start
              </button>
            </div>
            <div>
              ...
              <br />
              ={" "}
              {isNaN(timeLimitInMinutesOrNaN) ? (
                <span style={{ color: "red" }}>???</span>
              ) : (
                timeLimitInMinutesOrNaN.toFixed(1)
              )}{" "}
              minutes
            </div>
          </div>
